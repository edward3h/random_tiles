$ ->
  ...
  draw = ->
    ...
    tsize = Math.min(canvas.width / (cols * 2), canvas.height / rows)
    ...
    ps = [
      ...
      pattern(0, 1, "diagonal1", _int('wd1'))
    ...
    choose_color = (cell) ->
      ...
      ctx.drawImage(tiles[cmin].img, cell.x * tsize * 2 + cell.y * tsize, cell.y * tsize, tsize * 2, tsize)
  
    cells = []
  
    for x in [0..(cols - 1)]
      for y in [0..(rows - 1)]
        cells.push({x: x - Math.ceil(y / 2), y: y})        
